...
if __name__ == "__main__":
    ...
    exp = mlflow.set_experiment(experiment_name="my_elasticnet")
...
############################################################################
    exp = mlflow.set_experiment(experiment_name="my_lasso")
    print("Name: {}".format(exp.name))
    print("Experiment_id: {}".format(exp.experiment_id))
    print("Artifact Location: {}".format(exp.artifact_location))
    print("Tags: {}".format(exp.tags))
    ...
    # start run
    mlflow.start_run(run_name="lasso_r1")
    tags = {
        "engineering": "ML platform",
        "release.candidate": "RC1-Lasso",
        "release.version": "2.0"
    }
    mlflow.set_tags(tags)
    current_run = mlflow.active_run()
    ...
    # model code
    # first experiment- elastic net
    model = Lasso(alpha=0.5, max_iter=500)
    model.fit(X_train,y_train)
    prediction = model.predict(X_test)
    (mae,rmse,r2) = modelscore(y_test,prediction)
    ...
    # log model
    mlflow.sklearn.log_model(model, "runmodel-lasso")
    mlflow.log_artifacts("data/")
    artifacts_uri = mlflow.get_artifact_uri()
    print("The artifact path is", artifacts_uri)
    mlflow.end_run()
...
################## second run########################
# start run
    mlflow.start_run(run_name="lasso_r2")
    tags = {
        "engineering": "ML platform",
        "release.candidate": "RC2-lasso",
        "release.version": "2.1"
    }
    mlflow.set_tags(tags)
    current_run = mlflow.active_run()
    ...
    # model code
    model = Lasso(alpha=0.2,max_iter=300)
    model.fit(X_train,y_train)
    prediction = model.predict(X_test)
    (mae,rmse,r2) = modelscore(y_test,prediction)
    ...
    # log model
    mlflow.sklearn.log_model(model, "runmodel_1-lasso")
    mlflow.log_artifacts("data/")
    artifacts_uri = mlflow.get_artifact_uri()
    print("The artifact path is", artifacts_uri)
    mlflow.end_run()
#####################################################################
    exp = mlflow.set_experiment(experiment_name="my_ridge")
    print("Name: {}".format(exp.name))
    print("Experiment_id: {}".format(exp.experiment_id))
    print("Artifact Location: {}".format(exp.artifact_location))
    print("Tags: {}".format(exp.tags))
    ...
    # start run
    mlflow.start_run(run_name="run_ridge_1")
    tags = {
        "engineering": "ML platform",
        "release.candidate": "RC-ridge",
        "release.version": "2.0"
    }
    mlflow.set_tags(tags)
    current_run = mlflow.active_run()
    ...
    # log model
    mlflow.sklearn.log_model(model, "runmodel_ridge_1")
    mlflow.log_artifacts("data/")
    artifacts_uri = mlflow.get_artifact_uri()
    print("The artifact path is", artifacts_uri)
    mlflow.end_run()
...
################## second run########################
# start run
    mlflow.start_run(run_name="run_ridge_2")
    tags = {
        "engineering": "ML platform",
        "release.candidate": "RC2",
        "release.version": "2.1"
    ...
    # log model
    mlflow.sklearn.log_model(model, "runmodel_ridge-2")
    mlflow.log_artifacts("data/")
    artifacts_uri = mlflow.get_artifact_uri()
    print("The artifact path is", artifacts_uri)
    mlflow.end_run()
...
####################### Third run ####################
    ...
    # log model
    mlflow.sklearn.log_model(model, "runmodel_ridge_3")
    mlflow.log_artifacts("data/")
    artifacts_uri = mlflow.get_artifact_uri()
    print("The artifact path is", artifacts_uri)
    mlflow.end_run()
